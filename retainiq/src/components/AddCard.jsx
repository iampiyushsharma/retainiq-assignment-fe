  ...
  return (
    <>
      <div className="relative w-[300px] rounded shadow-lg p-4 border border-gray-200">
        <div className="relative w-full h-64 flex flex-col rounded-lg items-center justify-center group">
          {image ? (
            <>
              <img
                src={image[0]}
                alt="Uploaded Design"
                className="object-cover rounded-lg h-[95%] w-full"
              />
          ...
          {image && (
            <div
              className="absolute rounded-lg inset-0 flex items-center justify-center group-hover:flex opacity-0 group-hover:opacity-100 transition-opacity duration-300"
              onClick={() => setIsModalOpen(true)}
            >
